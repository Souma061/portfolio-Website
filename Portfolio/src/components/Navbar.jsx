          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                ...
                className="text-slate-300 hover:text-white transition-colors duration-300 font-medium text-sm relative group"
              >
                {link.name}
          ...
          <div className="px-2 pt-2 pb-3 space-y-1">
            {navLinks.map((link) => (
              <a
                ...
                href={link.href}
                onClick={(e) => handleScroll(e, link.href)}
                className="block px-3 py-2 rounded-md text-slate-300 hover:text-white hover:bg-slate-800 transition-colors duration-300 font-medium"
              >
                {link.name}
              </a>
            ))}
